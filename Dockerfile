...
RUN rm -f target/$RUST_TARGET/release/deps/tower_attack_cache*
COPY ./src ./src

RUN source $HOME/.cargo/env && \
    cargo build --release \
        --target="$RUST_TARGET" && \
    cp target/$RUST_TARGET/release/tower-attack-cache /tower-attack-cache && \
    actual-strip /tower-attack-cache

FROM scratch

COPY --from=builder /tower-attack-cache /tower-attack-cache

CMD ["./tower-attack-cache"]
